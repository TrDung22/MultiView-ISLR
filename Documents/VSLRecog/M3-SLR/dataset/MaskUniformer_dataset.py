...
class MaskUFThreeView_Dataset(Dataset):
    def __init__(self, base_url, split, dataset_cfg, **kwargs):
    ...
    def read_videos(self, center, left, right):
        index_setting = self.data_cfg['transform_cfg'].get('index_setting', ['consecutive', 'pad', 'central', 'pad'])
        #
        vlen1, _, _ = self.count_frames(os.path.join(self.base_url, 'crop_videos', center))
        vlen2, _, _ = self.count_frames(os.path.join(self.base_url, 'crop_videos', left))
        vlen3, _, _ = self.count_frames(os.path.join(self.base_url, 'crop_videos', right))
    ...
    def _gen_mask(self, mask_ratio, optical_flow=None):
        # Sinh mask theo config: sử dụng các tham số trong dataset_cfg cho MaskFeat.
        # Ví dụ, config cần có: 'mask_feat': { 'MASK_WINDOW_SIZE': [temporal, H, W], 'MASK_RATIO': 0.4 }
        mask_window_size = [8, 7, 7] 
    ...
    def __getitem__(self, idx):
        ...
        if self.data_cfg['motion_aware_masking']:
            frames_left = rgb_left.permute(0, 2, 3, 1).numpy()
            optical_flow_left = self.compute_optical_flow(frames_left)
            optical_flow_left = resize(optical_flow_left, (8, 7, 7), anti_aliasing=True)
            mask_left = self._gen_mask(self.data_cfg['mask_ratio_left'], optical_flow_left)

            frames_center = rgb_center.permute(0, 2, 3, 1).numpy()
            optical_flow_center = self.compute_optical_flow(frames_center)
            optical_flow_center = resize(optical_flow_center, (8, 7, 7), anti_aliasing=True)
            mask_center = self._gen_mask(self.data_cfg['mask_ratio_center'], optical_flow_center)

            frames_right = rgb_right.permute(0, 2, 3, 1).numpy()
            optical_flow_right = self.compute_optical_flow(frames_right)
            optical_flow_right = resize(optical_flow_right, (8, 7, 7), anti_aliasing=True)
            mask_right = self._gen_mask(self.data_cfg['mask_ratio_right'], optical_flow_right)
        else:
            mask_left = self._gen_mask(self.data_cfg['mask_ratio_left'],None) 
            mask_center = self._gen_mask(self.data_cfg['mask_ratio_center'],None) 
            mask_right = self._gen_mask(self.data_cfg['mask_ratio_right'],None) 
        return rgb_left, rgb_center, rgb_right, mask_left, mask_center, mask_right
