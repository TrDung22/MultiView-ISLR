...
def maskufthreeview_collate_fn_(batch):
    rgb_left = torch.stack([s[0] for s in batch], dim=0).permute(0,2,1,3,4)
    rgb_center = torch.stack([s[1] for s in batch], dim=0).permute(0,2,1,3,4)
    rgb_right = torch.stack([s[2] for s in batch], dim=0).permute(0,2,1,3,4)

    mask_left = torch.stack([s[3] for s in batch], dim=0)
    mask_center = torch.stack([s[4] for s in batch], dim=0)
    mask_right = torch.stack([s[5] for s in batch], dim=0)

    rgb_left = (rgb_left, mask_left)
    rgb_center = (rgb_center, mask_center)
    rgb_right = (rgb_right, mask_right)
    return {'rgb_left': rgb_left, 'rgb_center': rgb_center, 'rgb_right': rgb_right}
...
def build_dataloader(cfg, split, is_train=True, model = None,labels = None):
    ...
    if cfg['data']['model_name'] == 'MaskUFThreeView':
        collate_func = maskufthreeview_collate_fn_
